...
class Master extends DBConnection
{
		...
		if ($save) {
			if (isset($_FILES['img']) && count($_FILES['img']['tmp_name']) > 0) {
				if (!is_dir(base_app . 'uploads/services_' . $id)) {
					mkdir(base_app . 'uploads/services_' . $id);
					$data = " `upload_path`= 'uploads/servicese_{$id}' ";
				} else {
					$data = " `upload_path`= 'uploads/services_{$id}' ";
				}
				$this->conn->query("UPDATE `services` set {$data} where id = '{$id}' ");
				foreach ($_FILES['img']['tmp_name'] as $k => $v) {
					move_uploaded_file($_FILES['img']['tmp_name'][$k], base_app . 'uploads/services_' . $id . '/' . $_FILES['img']['name'][$k]);
				}
			}
	...
	function save_feedback()
	{
		...
		$data = "";
		foreach ($_POST as $k => $v) {
			if (!in_array($k, array('id', 'feedback'))) {
				if (!empty($data)) $data .= ",";
				$data .= " `{$k}`='{$v}' ";
			}
		}
		if (isset($_POST['feedback'])) {
			if (!empty($data)) $data .= ",";
			$data .= " `feedback`='" . addslashes(htmlentities($feedback)) . "' ";
		}
		if (empty($id)) {
			$sql = "INSERT INTO `user_feedback` set {$data} ";
			$save = $this->conn->query($sql);
			$id = $this->conn->insert_id;
		} else {
			$sql = "UPDATE `user_feedback` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if ($save) {
			if (isset($_FILES['img']) && count($_FILES['img']['tmp_name']) > 0) {
				if (!is_dir(base_app . 'uploads/feedback_' . $id)) {
					mkdir(base_app . 'uploads/feedback_' . $id);
					$data = " `upload_path`= 'uploads/feedback_{$id}' ";
				} else {
					$data = " `upload_path`= 'uploads/feedback_{$id}' ";
				}
				$this->conn->query("UPDATE `user_feedback` set {$data} where id = '{$id}' ");
				foreach ($_FILES['img']['tmp_name'] as $k => $v) {
					move_uploaded_file($_FILES['img']['tmp_name'][$k], base_app . 'uploads/feedback_' . $id . '/' . $_FILES['img']['name'][$k]);
				}
			}

			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', "New feedback successfully saved.");
		} else {
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error . "[{$sql}]";
		}
	...
	{
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `user_feedback` where id = '{$id}'");
		if ($del) {
			$resp['status'] = 'success';
			if (is_dir(base_app . 'uploads/feedback_' . $id)) {
				$file = scandir(base_app . 'uploads/feedback_' . $id);
				foreach ($file as $img) {
					if (in_array($img, array('..', '.')))
						continue;
					unlink(base_app . 'uploads/feedback_' . $id . '/' . $img);
				}
				rmdir(base_app . 'uploads/feedback_' . $id);
			}
			$this->settings->set_flashdata('success', "feedback successfully deleted.");
		} else {
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function delete_services()
	{
		...
		if ($del) {
			$resp['status'] = 'success';
			if (is_dir(base_app . 'uploads/services' . $id)) {
				$file = scandir(base_app . 'uploads/services' . $id);
				foreach ($file as $img) {
					if (in_array($img, array('..', '.')))
						continue;
					unlink(base_app . 'uploads/services' . $id . '/' . $img);
				}
				rmdir(base_app . 'uploads/services' . $id);
			}
	...
	function sbook_tour()
	{
		...
		$save = $this->conn->query("INSERT INTO `services_book` set $data");
		if ($save) {
			$resp['status'] = 'success';
		} else {
			$resp['status'] = 'failed';
	...
	function update_sbook_status()
	{
		extract($_POST);
		$update = $this->conn->query("UPDATE `services_book` set `status` = '{$status}' where id ='{$id}' ");
		if ($update) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', "Book successfully updated.");
		} else {
	...
	function save_orders()
	{
		...
		$save = $this->conn->query("INSERT INTO `orders` set $data");
		if ($save) {
			$resp['status'] = 'success';
		} else {
			$resp['status'] = 'failed';
	...
	{
		$del = $this->conn->query("DELETE FROM `orders` where id='{$_POST['id']}'");
		if ($del) {
			$resp['status'] = 'success';
	...
	function delete_sbooking()
	{
		$del = $this->conn->query("DELETE FROM `services_book` where id='{$_POST['id']}'");
		if ($del) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata("success", "Booking Deleted.");
		} else {
...
switch ($action) {
	...
	case 'update_book_status':
		...
		break;
	case 'update_sbook_status':
		echo $Master->update_sbook_status();
		break;
	case 'register':
	...
	case 'delete_booking':
		...
		break;
	case 'delete_sbooking':
		echo $Master->delete_sbooking();
		break;
	case 'delete_review':
