<section class="page-section">
    <div class="container">
        <div class="w-100 justify-content-between d-flex">
            ...
            <a href="./?page=edit_account" class="btn btn save_btn">
    ...
    <div class="container">
        ...
        <table class="table table-stripped text-dark  ">
            ...
            <tbody>
                <?php
                $i = 1;
                $qry = $conn->query("SELECT b.*,p.title FROM services_book b inner join `services` p on p.id = b.package_id where b.user_id ='" . $_settings->userdata('id') . "' order by date(b.date_created) desc ");
                while ($row = $qry->fetch_assoc()) :
                    $review = $conn->query("SELECT * FROM `rate_review` where package_id='{$row['id']}' and user_id = " . $_settings->userdata('id'))->num_rows;
                ?>
                    <tr>
...
<script>
    ...
    function cancel_sbook($id) {
        start_loader()
        $.ajax({
            url: _base_url_ + "classes/Master.php?f=update_sbook_status",
            method: "POST",
            data: {
                id: $id,
                status: 2
            ...
            success: function(resp) {
                if (typeof resp == 'object' && resp.status == 'success') {
                    alert_toast("service Book cancelled successfully", 'success')
                    setTimeout(function() {
                        location.reload()
                    }, 2000)
                } else {
    ...
    $(function() {
        $('.cancel_sdata').click(function() {
            _conf("Are you sure to cancel this service booking?", "cancel_sbook", [$(this).data('id')])
        })
        $('.submit_review').click(function() {
            uni_modal("Rate & Feedback", "./rate_review.php?id=" + $(this).data('id'), 'mid-large')
        })
