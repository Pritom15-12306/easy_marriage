...
include '../../config.php';
if (isset($_GET['id'])) {
    $qry = $conn->query("SELECT b.*,p.title,concat(u.firstname,' ',u.lastname) as name FROM services_book b inner join `services` p on p.id = b.package_id inner join users u on u.id = b.user_id where b.id = '{$_GET['id']}' ");
...
<p><b>Service:</b> <?php echo $title ?></p>
...
<script>
    $(function() {
        $('#sbook-status').submit(function(e) {
            e.preventDefault();
            start_loader()
            $.ajax({
